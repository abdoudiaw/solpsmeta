...
dataset = '../solps_diiid.db'
model = rf_learner.retrain(db_path=dataset)
torch.save(model, "rf_diiid.pt")

# Load model
print("PYTORCH VERSION", torch.__version__)
model = torch.load("rf_diiid.pt", weights_only=False)

# Load dataset
DB_PATH = '../solps_diiid.db'
raw_dataset = rf_learner.getAllGNDData(DB_PATH)
OUTPUT_SLICE = rf_learner.SOLVER_INDEXES["output_slice"]
...
for row in raw_dataset:
    input_sample = Inputs(*row[:7])
    pred, err = model(input_sample)
...
# Evaluate OK/Bad predictions
okay_prediction = np.array([
    list(model.iserrok(Outputs(*err))) for err in errbars
])

# Compute fuzzy relative error point-wise
...
plt.tight_layout()
plt.savefig("Figs/_histo_trueVSpredicted.png", dpi=300)
plt.show()
...
plt.tight_layout()
plt.savefig("Figs/_trueVSpredicted.png", dpi=300)
plt.show()
#
#
#
