...
class BGKModel(Model):
    def pack_inputs(self, request):
        # request is of type Inputs(gas_puff, p_tot, core_flux, dna, hci, r, region)
        return np.array([request.gas_puff, request.p_tot, request.core_flux,
    ...
    def pack_outputs(self, outputs):
        return np.array([outputs.ne, outputs.te, outputs.ti, outputs.po])
...
#    print("Targets shape:",targets.shape)
#    print(targets.std(axis=0))
#    return features, targets #torch.utils.data.TensorDataset(features,targets)

def assemble_dataset(raw_dataset):
    features = np.asarray(raw_dataset[:,SOLVER_INDEXES["input_slice"]])
    print("Feature shape:", features.shape)
    print(features.std(axis=0))
    targets = np.asarray(raw_dataset[:,SOLVER_INDEXES["output_slice"]])
    print("Targets shape:", targets.shape)
    print(targets.std(axis=0))
    return features, targets
...
#    full_dataset = assemble_dataset(raw_dataset, solver)
    ...
    #                                                 err_info=err_info)
#    model = Model(solver=None, forest=forest, uq_fussyness=learning_config['uq_fussyness'], err_info=err_info)

    model = BGKModel(solver=None, forest=forest, uq_fussyness=learning_config['uq_fussyness'], err_info=err_info)
